...
Section3:AddToggle({
    ...
    flag = "enable_esp",
    callback = function(state)
        library:SendNotification("ESP " .. (state and "Enabled" or "Disabled"), 6)
    end
})
...
Section3:AddToggle({
    ...
    flag = "show_name",
    callback = function(state)
        library:SendNotification("Show Name " .. (state and "Enabled" or "Disabled"), 6)
    end
})
...
Section3:AddToggle({
    ...
    flag = "show_box",
    callback = function(state)
        library:SendNotification("Show Box " .. (state and "Enabled" or "Disabled"), 6)
    end
})
...
Section3:AddToggle({
    ...
    flag = "show_distance",
    callback = function(state)
        library:SendNotification("Show Distance " .. (state and "Enabled" or "Disabled"), 6)
    end
})
...
Section3:AddToggle({
    ...
    flag = "show_weapon",
    callback = function(state)
        library:SendNotification("Show Weapon " .. (state and "Enabled" or "Disabled"), 6)
    end
})
...
Section3:AddToggle({
    ...
    flag = "show_health",
    callback = function(state)
        library:SendNotification("Show Health " .. (state and "Enabled" or "Disabled"), 6)
    end
})
...
Section3:AddToggle({
    ...
    flag = "show_tracer",
    callback = function(state)
        library:SendNotification("Show Tracer " .. (state and "Enabled" or "Disabled"), 6)
    end
})
...
Section3:AddToggle({
    ...
    flag = "show_head",
    callback = function(state)
        library:SendNotification("Show Head " .. (state and "Enabled" or "Disabled"), 6)
    end
})

Section3:AddToggle({
    text = "Enable Skeletons",
    flag = "enable_skeletons",
    callback = function(state)
        library:SendNotification("Skeletons " .. (state and "Enabled" or "Disabled"), 6)
    end
})
...
Section5:AddToggle({
    text = "Speed Hack",
    flag = "speed_hack_toggle",
    callback = function(state)
        library:SendNotification("Speed Hack " .. (state and "Enabled" or "Disabled"), 6)
        if speed_hack.speedhack then
            speed_hack.speedhack.Enabled = state and library.flags.speed_hack_bind
        end
    end
})
...
Section5:AddBind({
    ...
    tooltip = "Hold to activate Speed Hack (requires Speed Hack toggle enabled)",
    mode = "hold",
    bind = "Q",
    ...
    callback = function(state)
        ...
        if speed_hack.speedhack then
            speed_hack.speedhack.Enabled = library.flags.speed_hack_toggle and state
        end
    end
})
...
Section5:AddToggle({
    ...
    flag = "fly_toggle",
    callback = function(state)
        library:SendNotification("Fly " .. (state and "Enabled" or "Disabled"), 6)
    end
})
...
Section5:AddBind({
    enabled = true,
    text = "Fly Keybind",
    tooltip = "Hold to activate Fly (requires Fly toggle enabled)",
    mode = "hold",
    bind = "E",
    flag = "fly_bind",
    state = false,
...
Section5:AddToggle({
    text = "Manipulation",
    flag = "manipulation_toggle",
    callback = function(state)
        library:SendNotification("Manipulation " .. (state and "Enabled" or "Disabled"), 6)
        if not state then
            ManipulationReset()
        end
    end
...
Section5:AddBind({
    ...
    tooltip = "Toggle to activate Manipulation",
    mode = "toggle",
    bind = "M",
...
Section5:AddToggle({
    ...
    flag = "noclip",
    callback = function(state)
        library:SendNotification("No Clip " .. (state and "Enabled" or "Disabled"), 6)
    end
})
...
Section5:AddBind({
    ...
    tooltip = "Hold to activate No Clip (requires No Clip toggle enabled)",
    mode = "hold",
    bind = "N",
    flag = "noclip_key",
    state = false,
...
Section6:AddToggle({
    text = "Free Cam",
    flag = "free_cam_toggle",
    callback = function(state)
        library:SendNotification("Free Cam " .. (state and "Enabled" or "Disabled"), 6)
    end
})
...
Section6:AddBind({
    ...
    tooltip = "Hold to activate Free Cam",
    mode = "hold",
    bind = "F",
    flag = "free_cam_bind",
    state = false,
...
Section6:AddSlider({
    text = "Free Cam Sensitivity",
    flag = "free_cam_sensitivity",
    min = 0,
    max = 1,
    value = 0.5,
    callback = function(value)
...
Section6:AddToggle({
    ...
    callback = function(state)
        library:SendNotification("Third Person " .. (state and "Enabled" or "Disabled"), 6)
    end
})
...
Section6:AddBind({
    ...
    tooltip = "Hold to activate Third Person view",
    mode = "hold",
    bind = "T",
...
Section6:AddToggle({
    text = "Camera FOV",
    flag = "camera_fov_toggle",
    callback = function(state)
        library:SendNotification("Camera FOV " .. (state and "Enabled" or "Disabled"), 6)
    end
})

Section6:AddSlider({
    text = "Camera FOV Value",
...
Section6:AddToggle({
    ...
    flag = "zoom_toggle",
    callback = function(state)
        library:SendNotification("Zoom " .. (state and "Enabled" or "Disabled"), 6)
    end
})
...
Section6:AddBind({
    enabled = true,
    text = "Zoom Keybind",
    tooltip = "Hold to activate Zoom (requires Zoom toggle enabled)",
    mode = "hold",
    bind = "Z",
    flag = "zoom_bind",
    state = false,
...
-- Adding toggles and slider to Section7 (Gun Mods)
Section7:AddToggle({
    text = "No Recoil",
        ...
        for _, tbl in getgc(true) do
            ...
            for _, key in pairs({"FireRate", "RateOfFire", "ShotDelay", "Cooldown"}) do
                ...
                if value and type(value) == "number" then
                    if not gun_mods.old_fire_rate[tbl] then
                        gun_mods.old_fire_rate[tbl] = {}
                    end
...
-- Enhanced ESP Logic with Ultra-Tight Boxes and Inner Head Circle
local local_player = Players.LocalPlayer
local camera = Workspace.CurrentCamera
local viewport_size = camera and camera.ViewportSize or Vector2.new(1920, 1080)
...
local function update_fly()
    ...
    if library.flags.fly_toggle and library.flags.fly_bind then
        ...
        if UserInputService:IsKeyDown(Enum.KeyCode.W) then
            move_direction = move_direction + camera.CFrame.LookVector
        end
        if UserInputService:IsKeyDown(Enum.KeyCode.S) then
            move_direction = move_direction - camera.CFrame.LookVector
        end
        if UserInputService:IsKeyDown(Enum.KeyCode.A) then
            move_direction = move_direction - camera.CFrame.RightVector
        end
...
function player_esp.functions.get_player()
    ...
    for _, player in pairs(Players:GetPlayers()) do
        ...
        -- Team check (modify based on game's team system)
        ...
        local ray = Ray.new(camera_pos, (head_pos - camera_pos).Unit * 1000)
        local ignore = {local_player.Character}
...
local function update_aimbot()
    ...
    highlight_player = target_enhancements.target.entry
...
do
    ...
    FieldOfView.Parent = cloneref(game:GetService("CoreGui"))
...
local function init_speed_hack()
    ...
    local success, result = pcall(function()
        ...
        speedhack.Parent = cloneref(game:GetService("CoreGui"))
        ...
        if plr_model then
            for _, child in pairs(plr_model:GetDescendants()) do
                if child:IsA("BasePart") then
                    if library.flags.noclip_key then
...
local camera_settings = {
    default_fov = Workspace.CurrentCamera and Workspace.CurrentCamera.FieldOfView or 70,
    default_subject = nil,
    default_type = nil,
}
...
local function update_camera()
    ...
    if free_cam.enabled or third_person.enabled then
        return
    end
        ...
        if on_screen then
            ...
            if on_screen and library.flags.enable_skeletons and character and character:FindFirstChild("UpperTorso") and character:FindFirstChild("HumanoidRootPart") then
                ...
                if not all_parts_valid then
                    ...
                    linepos(
                        drawings.parts.UpperTorso_LowerTorso,
                        positions.UpperTorso,
                        positions.LowerTorso,
                        library.flags.skeleton_thickness,
                        root
                    )
                    linepos(
                        drawings.parts.UpperTorso_LeftUpperArm,
                        positions.UpperTorso,
                    ...
                    linepos(
                        drawings.parts.LeftUpperArm_LeftLowerArm,
                        positions.LeftUpperArm,
                    ...
                    linepos(
                        drawings.parts.LeftLowerArm_LeftHand,
                        positions.LeftLowerArm,
                        positions.LeftHand,
                        library.flags.skeleton_thickness,
                        root
                    ...
                    linepos(
                        drawings.parts.UpperTorso_RightUpperArm,
                        positions.UpperTorso,
                    ...
                    linepos(
                        drawings.parts.RightUpperArm_RightLowerArm,
                        positions.RightUpperArm,
                    ...
                    linepos(
                        drawings.parts.RightLowerArm_RightHand,
                        positions.RightLowerArm,
                        positions.RightHand,
                        library.flags.skeleton_thickness,
                        root
                    ...
                    linepos(
                        drawings.parts.LowerTorso_LeftUpperLeg,
                        positions.LowerTorso,
                    ...
                    linepos(
                        drawings.parts.LeftUpperLeg_LeftLowerLeg,
                        positions.LeftUpperLeg,
                    ...
                    linepos(
                        drawings.parts.LeftLowerLeg_LeftFoot,
                        positions.LeftLowerLeg,
                        positions.LeftFoot,
                        library.flags.skeleton_thickness,
                        root
                    ...
                    linepos(
                        drawings.parts.LowerTorso_RightUpperLeg,
                        positions.LowerTorso,
                    ...
                    linepos(
                        drawings.parts.RightUpperLeg_RightLowerLeg,
                        positions.RightUpperLeg,
                    ...
                    linepos(
                        drawings.parts.RightLowerLeg_RightFoot,
                        positions.RightLowerLeg,
                        positions.RightFoot,
                        library.flags.skeleton_thickness,
                        root
...
if hookfunction then
    ...
    library:SendNotification("Warning: GetService hook not supported by this executor", 6)
end

local old_checkcaller
if hookfunction and checkcaller then
    old_checkcaller = hookfunction(checkcaller, function()
        return true
    end)
else
    library:SendNotification("Warning: checkcaller hook not supported by this executor", 6)
end
    ...
    old_index = hookmetamethod(game, "__index", function(self, key)
        ...
        if not checkcaller() and key == "GetService" then
            return function(class)
                return game:FindService(class) or error("Service not found: " .. tostring(class))
            end
        end
        return old_index(self, key)
    end)
else
    library:SendNotification("Warning: __index hook not supported by this executor", 6)
end

local old_newindex
if hookmetamethod then
    old_newindex = hookmetamethod(game, "__newindex", function(self, key, value)
        if not checkcaller() and self:IsA("Humanoid") and (key == "WalkSpeed" or key == "Health") then
            return
        end
        return old_newindex(self, key, value)
    end)
else
    library:SendNotification("Warning: __newindex hook not supported by this executor", 6)
end
...
if hookmetamethod then
    ...
    library:SendNotification("Warning: Anti-ban __namecall hook not supported by this executor", 6)
end
...
-- Update ESP, Fly, Manipulation, Aimbot, Free Cam, Third Person, Camera, Speed Hack, No-Clip, and FOV Circle every frame
...
game:BindToClose(function()
    ...
    if remake.distanceText then
        remake.distanceText:Remove()
    end
    if remake.diedConnection then
        remake.diedConnection:Disconnect()
    end
    ...
    if speed_hack.speedhack and speed_hack.speedhack.Parent then
        speed_hack.speedhack:Destroy()
    end
    if fov_circle.FieldOfView then
        fov_circle.FieldOfView:Destroy()
    end
    for _, connection in ipairs(activeConnections) do
        connection:Disconnect()
    end
